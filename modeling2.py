...
class BertCapsule(BertModel):
	...
	def get_cnn_output(self):
		...
		print("_____________ batch_size value:{} ____________".format(self.batch_size))
		...
		with tf.variable_scope('Second_caps_layer'):
			secondCaps = CapsLayer(num_outputs=4, vec_len=8, layer_type='FC', with_routing=True, bz=self.batch_size)
			self.caps2 = secondCaps(caps1, kernel_size=3, stride=1)

		with tf.variable_scope('LSTM_layer'):
			caps2_reshape = tf.reshape(self.caps2, [self.batch_size, 4, -1])
...
class BertTextcnn(BertModel):
		...
		for i, filter_size in enumerate(self.filter_sizes):
			with tf.name_scope("conv-maxpool-%s" % filter_size):
				...
				pooled = tf.nn.max_pool(
					h,
					ksize=[1, sequence_output.shape[1] - filter_size + 1, 1, 1],
					strides=[1, 1, 1, 1],
					padding='VALID',
					name="pool")
	...
	#3.进行dropout
		...
		with tf.name_scope("dropout"):
			...
			self.h_drop = dropout(self.h_pool_flat, 1.0 - self.dropout_keep_prob, seed=self.config.random_seed)
	...
	def get_model_output(self):
		...
		output_weights = tf.get_variable(
				...
				initializer=tf.truncated_normal_initializer(stddev=0.02, seed=self.config.random_seed))
...
def dropout(input_tensor, dropout_prob, seed=0):
	"""Perform dropout.

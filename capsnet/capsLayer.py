	...
	def __call__(self, input, kernel_size=None, stride=None):
		...
		if self.layer_type == 'CONV':
			...
			if not self.with_routing:
				...
				capsules = tf.reshape(capsules, (cfg_batch_size, -1, self.vec_len, 1))
...
def routing(input, b_IJ):
	W = tf.get_variable('Weight', shape=(1,1152,4,8,16), dtype=tf.float32,     # * **
						initializer=tf.random_normal_initializer(stddev=cfg_stddev))
	biases = tf.get_variable('bias', shape=(1,1,1,16,1))

	input = tf.tile(input, [1,1,4,1,1])    # **
	W = tf.tile(W, [cfg_batch_size,1,1,1,1])

	u_hat = tf.matmul(W, input, transpose_a=True)

	u_hat_stopped = tf.stop_gradient(u_hat, name='stop_gradient')

	for r_iter in range(cfg_iter_routing):
		with tf.variable_scope('iter_' + str(r_iter)):
			c_IJ = tf.nn.softmax(b_IJ, dim=2)

			if r_iter == cfg_iter_routing - 1:
				s_J = tf.multiply(c_IJ, u_hat)
				s_J = tf.reduce_sum(s_J, axis=1, keep_dims=True) + biases

				v_J = squash(s_J)

			elif r_iter < cfg_iter_routing - 1:
